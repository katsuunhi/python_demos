...
def finish():
   ...
   if not notEmpty:
       tk.messagebox.showerror(title='Error', message='Please input three edges!')
   else:
       edge1=int(t1.get())
       edge2=int(t2.get())
       edge3=int(t3.get())
   if edge1+edge2>edge3 and edge1+edge3>edge2 and edge2+edge3>edge1:
       ...
       tk.messagebox.showinfo(title='triangle', message='This is a triangle!\n\nArea is '+str(area)+'\n\nPerimeter is ' + str(perimeter))
   else:
       tk.messagebox.showwarning(title='Warning', message='This is not a triangle!')
